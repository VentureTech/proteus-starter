...
import static com.example.app.profile.ui.user.MembershipOperationsEditorUILOK.MEMBERSHIP_UI_HEADING_FORMAT;
 ...
 */
@I18NFile(
    symbolPrefix = "com.example.app.profile.ui.user.MembershipOperationsEditorUI",
    i18n = {
...
class MembershipOperationsEditorUI extends Container
{
    ...
    public void init()
    {
        ...
        _searchUI = new SearchUIImpl<MembershipOperation>(options);
    ...
    private SearchSupplierImpl<MembershipOperation> getSearchSupplier()
    {
        ...
        searchSupplier.setBuilderSupplier(() -> {
            final QLBuilderImpl<ProfileType> qb = new QLBuilderImpl<>(ProfileType.class, "ptAlias");
            qb.appendCriteria("id", Operator.eq, _membership.getProfile().getProfileType().getId());
            final JoinedQLBuilder<MembershipType> mtQB = qb.createInnerJoin(ProfileType.MEMBERSHIP_TYPES_PROP);
            final JoinedQLBuilder<MembershipOperation> opQB = mtQB.createInnerJoin(MembershipType.DEFAULT_OPERATIONS_PROP);
